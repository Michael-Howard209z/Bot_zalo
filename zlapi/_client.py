...
class ZaloAPI(object):
    def __init__(self, phone, password, imei, session_cookies=None, user_agent=None, auto_login=True):
        ...
        if auto_login:
            if session_cookies:
                self.setSession(session_cookies)
                self._state.login(
                    phone,
                    password,
                    imei,
                ...
                if not self.isLoggedIn():
                    raise _exception.ZaloLoginError(
                        "Session cookie không hợp lệ hoặc đã hết hạn."
                    )
            else:
    ...
    def sendRemoteVideo(self, videoUrl, thumbnailUrl, duration, thread_id, thread_type, width=1280, height=720, message=None, ttl=0):
        ...
        error_message = data.get("error_message") or data.get("data")
        raise ZaloAPIException(f"Error #{error_code} when sending requests: {error_message}")
    # dcm sendremote voice chứ đéo phải sendlo
    ...
    def _listen_ws(self, thread=False, reconnect=5):
        ...
        ws = None
        for url in self._state._config["zpw_ws"]:
            parsed = urllib.parse.urlsplit(url)
            hostname = parsed.hostname or ""
            port = parsed.port or 443
            try:
                raw_sock = socket.create_connection((hostname, port), timeout=8)
                raw_sock.settimeout(8)
                if parsed.scheme == "wss":
                    raw_sock = ssl.create_default_context().wrap_socket(raw_sock, server_hostname=hostname)
            except Exception as err:
                logger.warning("Skip %s: %s", url, err)
                continue
            try:
                ws_url = f"ws://{hostname}:{port}/?" + query
                ws = connect(
                    ws_url,
                    sock=raw_sock,
                    proxy=False,
                ...
                logger.warning("Failed to connect to %s: %s", url, err)
                try:
                    raw_sock.close()
                except Exception:
                    pass
        ...
        if ws is None:
            raise ZaloUserError("Unable to connect to any Zalo websocket server")
