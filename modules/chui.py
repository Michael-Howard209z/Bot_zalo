...
def handle_chui_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    try:
        with open("noidungg.txt", "r", encoding="utf-8") as file:
            Ngon = file.readlines()
    except FileNotFoundError:
        client.replyMessage(
            Message(text="Không tìm thấy file noidungg.txt."),
            message_object,
            thread_id,
            thread_type
        )
    ...
    if not Ngon:
        client.replyMessage(
            Message(text="File noidungg.txt không có nội dung nào để gửi."),
            message_object,
            thread_id,
            thread_type
        )
