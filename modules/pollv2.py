...
def handle_warpoll_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    try:
        ...
        username = "Người dùng không xác định"
    try:
        file_path = os.path.join("modules", "cache", "noidung.txt")
        with open(file_path, "r", encoding="utf-8") as file:
            captions = file.readlines()
            captions = [caption.strip() for caption in captions if caption.strip()]
    except FileNotFoundError:
        client.replyMessage(Message(text="Không tìm thấy file noidung.txt"), message_object, thread_id, thread_type)
        return
    if not captions:
        client.replyMessage(Message(text="File noidung.txt không có nội dung nào để gửi."), message_object, thread_id, thread_type)
        return
    is_polling = True
    ...
    def poll_loop():
        ...
        while is_polling:
            ...
            try:
                client.createPoll(question=question, options=["WARR CONCAC TAO TEST ", "🐤"], groupId=thread_id)
...
def get_mitaizl():
    return {
        'warpollv2': handle_warpoll_command
    }