...
des = {
    ...
    'description': "trò chuyện với Nextor"
}
...
def handle_sim_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if len(text) < 2:
        error_message = Message(text="𝐗𝐢𝐧 𝐂𝐡𝐚̀𝐨 𝐁𝐚̣𝐧 ! 𝐓𝐨̂𝐢 𝐥𝐚̀ 𝗡𝗲𝘅𝘁𝗼𝗿\n𝙷𝚊̃𝚢 𝚍𝚞̀𝚗𝚐 infoniu đ𝚎̂̉ 𝚋𝚒𝚎̂́𝚝 𝚝𝚑𝚎̂𝚖 𝚗𝚑𝚎́ >< 🐰")
    ...
    try:
        ...
        data = response.json()
        simi = data.get('answer', 'Không có phản hồi từ Nextor.')
        message_to_send = Message(text=f"> niu day: {Nextor}")
