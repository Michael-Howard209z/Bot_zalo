...
des = {
    ...
    'description': "Gửi nội dung từ file 2.txt liên tục trong nhóm."
}
...
def handle_war_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id not in ADMIN:
        client.replyMessage(
            Message(text="Phải là Hw Dzi mới sử dụng được lệnh này."),
    ...
    if len(command_parts) < 2:
        client.replyMessage(Message(text="Vui lòng chỉ định lệnh hợp lệ (vd: 2c on hoặc 2c stop)."), message_object, thread_id, thread_type)
        return
