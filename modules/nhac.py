...
BACKGROUND_PATH = "modules/background/"
...
des = {
    "version": "1.0.0",
    "credits": "Nhạc scl",
    "description": "tìm và gửi nhạc từ scl <prefix>scl",
...
def generate_menu_image(self, author_id, thread_id, thread_type):
    ...
    try:
        ...
        greeting_name = "Chủ Nhân" if is_admin(author_id) else user_name
        ...
        text_lines = [
            ...
            f"{PREFIX}scl on/off: Bật/Tắt tính năng",
...
def convert_to_m4a(file_path):
    """Chuyển mp3 sang m4a (AAC) vì player voice của Zalo chỉ phát được m4a, không phát mp3 bên ngoài."""
    try:
        ffmpeg = get_ffmpeg_path()
        if not ffmpeg:
            print("[DEBUG] Không tìm thấy ffmpeg, bỏ qua chuyển đổi m4a")
            return file_path

        base, _ = os.path.splitext(file_path)
        out_path = base + ".m4a"

        cmd = [ffmpeg, "-y", "-i", file_path, "-vn", "-c:a", "aac", "-b:a", "128k", out_path]
        result = subprocess.run(cmd, capture_output=True, timeout=600)

        if result.returncode != 0 or not os.path.exists(out_path):
            print(f"[DEBUG] Chuyển đổi m4a thất bại: {result.stderr[-200:] if result.stderr else ''}")
            return file_path

        os.remove(file_path)
        print(f"[DEBUG] Đã chuyển đổi m4a: {out_path}")
        return out_path
    except Exception as e:
        print(f"[DEBUG] Lỗi chuyển đổi m4a: {e}")
        return file_path
...
def create_song_list_image(songs):
    try:
        ...
        if not background_images:
            print(" đíu có ảnh trong mục background/")
...
def create_single_song_image(song):
    try:
        ...
        text_y = draw_gradient_text_line(draw, " Nền tảng: SoundCloud ☁️", text_x, text_y, font, emoji_font)
        text_y = draw_gradient_text_line(draw, " Chill cùng nhạc thôi nào", text_x, text_y, font, emoji_font)
        draw_gradient_text_line(draw, f"{plays:,}   {likes:,}   {comments:,}", text_x, text_y, font, emoji_font)
...
def handle_nhac_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    reactions = ["😍", "❤️", "/-ok", "👋", "🫠", "/-strong", "😂", "😉", "🥳", "🤩", "Dạ", "Yew", "Yes", "Ok", "Oki", "Uwu", "😃", "🥰", "/-ok", "/-heart"]
    ...
    if len(content) == 2 and content[0].lower() == f"{PREFIX}scl" and content[1].isdigit():
...
Chờ Lấy Nhạc Nhé..."""
    ...
    if len(content) < 2:
        print("[DEBUG] Không có nội dung tìm kiếm, hiển thị menu gợi ý.")
        if not author_id:
            author_id = "Đéo Biết"
        ...
        try:
            ...
            print(f"[ERROR] Lỗi khi tạo ảnh menu: {e}")
            image_path = None

        greeting_name = "Chủ Nhân" if is_admin(author_id) else username

        caption = f"""{username}
   ➜Vui lòng nhập tên bài hát để tìm kiếm sau lệnh {PREFIX}scl 
   ➜ Ví dụ: {PREFIX}scl dừng thương
"""
    ...
    if image_path:
        try:
            with Image.open(image_path) as img:
                width, height = img.size
                text = f"{username}, Nhập {PREFIX}Scl <số> để chọn bài \n Ví Dụ: {PREFIX}scl 3 "
                mention = Mention(author_id, offset=2, length=len(username)) if thread_type != ThreadType.USER else None
                msg = Message(text=text, mention=mention)
